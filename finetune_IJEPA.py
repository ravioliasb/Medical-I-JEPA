...
class IJEPADataset(Dataset):
    def __init__(self, dataset_path, stage='train', transform=None):
        super().__init__()
        if stage == 'train':
...
class D2VDataModule(pl.LightningDataModule):
    def __init__(self, dataset_path, batch_size=128, num_workers=4, pin_memory=True, shuffle=True):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        self.test_dataset = IJEPADataset(dataset_path=self.dataset_path, stage='test')
...
if __name__ == '__main__':
    dataset = D2VDataModule(dataset_path='/projects/earlydetection/Sagittal')
    model = IJEPA_FT(pretrained_model_path='pretrain-checkpoints-Med/last.ckpt', num_classes=11)

    logger = TensorBoardLogger("lightning_logs", name="IJEPA_finetune_logs")
    ...
    model = IJEPA_FT.load_from_checkpoint(best_checkpoint_path, num_classes=11)
