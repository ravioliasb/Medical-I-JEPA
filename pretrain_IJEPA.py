...
class D2VDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        #subset_size = val_size // 200
        #indices = np.random.choice(val_size, subset_size, replace=False)
...
if __name__ == '__main__':
    dataset = D2VDataModule(dataset_path='/projects/earlydetection/imagenet')


    model = IJEPA(img_size=224, patch_size=16, in_chans=3, embed_dim=64, enc_heads=8, enc_depth=8, decoder_depth=6, lr=1e-3)

    logger = TensorBoardLogger("lightning_logs", name="IJEPA_pretrain_logs")
